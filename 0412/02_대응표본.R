...
if(t_test_result$p.value < 0.05){
  ...
  print('결론 : 다이어트 프로그램 효과가 통계적으로 유의하지 않습니다.')
}
...
# 20대 운동 전 후 감소가 유의한 효과가 있는지 대응표본으로 검증
library(dplyr)
...
# !is.na : 결측값이 아닌
data=exercise_data%>%
  filter(substr(exercise_data$Age, 1,1)==2 & !is.na(Post_Weight)) 
#View(data) #결측값이 제거된 20대 데이터

t_test_result=t.test(data$Pre_Weight, data$Post_Weight, paired =TRUE)

if(t_test_result$p.value <0.05){
  print('다이어트 프로그램 체중 감소가 있습니다.')
}else{
  print('다이어트 체중 감소가 없습니다.')
}
