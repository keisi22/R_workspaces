...
# 그중 80개는 맛있는 사과고 20개는 맛없는 사과
# 우리는 상자에서 10개(표본집단)만 뽑아서 맛있는 사과의 비율을 추정하려고 합니다.

# 10개를 뽑았을 때 맛있는 사과의 비율이 항상 80%일까?
# 표본을 무작위로 뽑기 때문에 어떤 경우에는 7개가 맛있고,
# 또 어떤 경우에는 9개가 맛있는 사과일 수도 있음
# 즉, 표본 비율(맛있는 사과 비율)은 매법 조금씩 달라질 수 있음
...
# 약 ±12.65% 범위내에서 변동할 가능성
# 대략 67.35% ~ 92.65% 사이에 있을 가능성이 높다

# 신뢰구간
# 추가로 신뢰구간을 계산하면 더 명확하게 범위를 측정
# 신뢰구간?
# 날씨앱에서 '내일 비 올 확률 70%!' 라고 알려줌
# 70% ±5% 라고 구간을 알려줌
# 신뢰구간은 추측한 값이 실제로 있을 법한 범위를 알려준다.
...
# 우리가 뽑은 표본의 결과를 바탕으로
# 모집단의 맛있는 사과 비율은 약 55% ~ 105% 사이에 있을 가능성이 있다고
# 말할 수 있다.
